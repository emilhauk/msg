...
`video/quicktime` (iPhone `.mov`, usually HEVC) bypasses presign. Client POSTs the raw file to `/rooms/{id}/transcode` (session cookie auth, `Content-Type: video/*`, max **100 MiB**). Server buffers to a temp dir, runs `ffmpeg` (libx264 veryfast, crf 23, aac, 4 threads, capped at 1920px wide, `+faststart`), `PutObject`s the result to S3 as `video/mp4` under the normal media key, and returns attachment JSON. Synchronous: the request stays open for the whole transcode. Response is `200` `application/x-ndjson`: a bare `\n` heartbeat every 10 s while ffmpeg runs (keeps iOS Safari's 60 s idle timeout and proxies from dropping the request), then one JSON line — the attachment, or `{"error":"…"}` on failure. Client parses the last line; the upload chip spinner covers the wait. Two concurrent ffmpeg jobs; others queue in-process. `ffmpeg` must be on `PATH` (installed in both Dockerfiles); `handler.FFmpegPath` is overridable for tests.
